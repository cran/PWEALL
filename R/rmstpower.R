...
rmstpower<-function(tfix=2,tcut=seq(tfix,tfix+2,by=0.5),alpha=0.05,twosided=1,taur=1.2,u=c(1/taur,1/taur),ut=c(taur/2,taur),pi1=0.5,
                     ...
                     n=1000) {
  ##tfix: time at which RMSTs are calculated
  ##tcut: time points (from FPI) where powers are calculated 
  ##alpha: alpha level
  ...
  for (i in 1:nt){
    ...
    bb<-rmstutil(tfix=tfix,tcut=t1,taur=taur,u=u,ut=ut,
    ...
    aa<-rmstutil(tfix=tfix,tcut=t1,taur=taur,u=u,ut=ut,
                 rate1=r1,rate2=r2,rate3=r3,rate4=r4,rate5=r5,ratec=rc,
  ...
  list(power=power,rmst1=rmst1,se1=sqrt(vrmst1),rmst0=rmst0,se0=sqrt(vrmst1),drmst=drmst,sed=sqrt(vdrmst))
}
