...
SUBROUTINE FOURHR(NT,NR,TIN,R1,R2,R3,R4,TCHANGE,EPS,FX)
!DEC$ ATTRIBUTES DLLEXPORT,C,REFERENCE,ALIAS:'fourhr_' ::FOURHR
   IMPLICIT NONE
   INTEGER,INTENT(IN)::NT,NR
   REAL(8),INTENT(IN)::TIN(NT),R1(NR),R2(NR),R3(NR),R4(NR),TCHANGE(NR),EPS
   ...
   IF (N1>0) THEN
      ...
      CALL PWE(NR,NR,TCHANGE,R2,TCHANGE,PR2)
      CALL PWE(NR,NR,TCHANGE,R4,TCHANGE,PR4)
